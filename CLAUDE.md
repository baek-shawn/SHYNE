...
지금은 Step 1(Agent Loop + 최소 Tool 6종) 구현 직전 단계이며, 코드는 아직 없고 디렉터리 구조와
문서 체계만 갖춰져 있습니다. 현재 진행 상태는 항상 [`.agent/HANDOFF.md`](.agent/HANDOFF.md)를
기준으로 판단하세요.
...
```text
backend/
├── main.py           # 진입점 (Step 2에서 FastAPI 앱 구성)
